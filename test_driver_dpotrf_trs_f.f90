...
use Sub_CPU_Interfaces
    ...
    integer  :: n, n1,m,  Nc,k,L, Nb, Lmax,nrhs, ngpu
...
!! ------ control
      ...
      integer   :: ceil
      DOUBLE PRECISION, ALLOCATABLE :: TIME_ITER_TRF(:),TIME_ITER_TRS(:),TIME_ITER_mm(:),TIME_ITER_MMsyrk(:)
      DOUBLE PRECISION, ALLOCATABLE :: TIME_ITER_TRS_RHS(:,:)
      DOUBLE PRECISION, ALLOCATABLE :: RESID_RHS(:,:),derror_RHS(:,:),SM_abs_AX_B(:,:)
      DOUBLE PRECISION, ALLOCATABLE :: MenLoc_rhs(:)
      
      integer, ALLOCATABLE :: nb_rhs(:)
      integer, ALLOCATABLE :: L_rhs(:)
...
!     /*     Write table header to output
         ...
         WRITE( NOUT,   *          ) '# DPOTRF & DPOTRS performance results, pogram=',trim(PROGNAM)
         WRITE( NOUT,   *          ) '# Par in:'
...
!         write(NOUT,514) "n","jNrhs","LDA","it","#OK","Erro","RESID","S_AB(AX-B)","tfac","tslv","tsolT", &
!                            "GFPS_F","GFPS_S","GFPS_SV","GFPS_Mrk","GFPS_SMM","M_f(MB)","M_s(MB)","TM (GB)"
         write(NOUT,515) "# n","jNrhs","LDA","Nb","maxRHS","L","it","#OK","Erro","RESID","S_AB(AX-B)","tfac","tslv","tsolT", &
                            "GFPS_F","GFPS_S","GFPS_SV","GFPS_Mrk","GFPS_SMM","M_f(MB)","M_s(MB)","TM (GB)"
...
if (ejecutar .eqv. .true.) then
  ...
  do N = nmin , nmax , nstep
    ...
    ALLOCATE( TIME_ITER_TRF( ITERATIONS ) )
    ALLOCATE( TIME_ITER_TRS( ITERATIONS ) )
    ALLOCATE( TIME_ITER_mm( ITERATIONS ) )
    ALLOCATE( TIME_ITER_mmsyrk( ITERATIONS ) )
    ALLOCATE( TIME_ITER_TRS_RHS( ITERATIONS,NNRHS ) )
    ...
    ALLOCATE( nb_rhs(NNRHS))
    ALLOCATE( L_rhs(NNRHS))
...
!print *, ""
    ...
    MenLocSET=MenLoc
            ...
            !print *, ""
	 !print *, "++++ Star   magmaf_dpotrf_m "
         !T1 = MPI_WTIME()
         T1 =omp_get_wtime()

         ngpu=1
        call magmaf_dpotrf_m(ngpu,'L',n, A, lda, info) !!! hibrid, out of core, original magma

         !T2 = MPI_WTIME()
	    T2 =omp_get_wtime()
         !print *, "++++ finish magmaf_dpotrf_m "
        !print *, ""
        !print *, " time magmaPoTRF ", T2-T1," s"
         tfac=t2-t1
         TIME_ITER_TRF( LNCH ) = tfac
...
!!!!!!!!!!!   N RHS
    ...
    do k = kmin , kmax , kstep   !!  jnrhs  !! loop 
      ...
      opcB=3
		...
		if(opcB==1)then  !!! RHS
		        MenLoc0=((real(LDB)*real(jnrhs))*bytes*4.0d0)/B2GB ; MenLoc=MenLocSET+MenLoc0
                MenLoc0=((real(m)*real(jnrhs))*bytes*1.0d0)/B2GB ; MenLoc=MenLoc+MenLoc0

		elseif(opcB==2)then  !! direct gem sym matrix
			    MenLoc0=((real(LDB)*real(jnrhs))*bytes*4.0d0)/B2GB ; MenLoc=MenLocSET+MenLoc0

		elseif(opcB==3)then  !! b random
			    MenLoc0=((real(LDB)*real(jnrhs))*bytes*1.0d0)/B2GB ; MenLoc=MenLocSET+MenLoc0
		end if
   ...
   if(MenLoc<450.0d0 .and. ejecutar.eqv. .true. )then   
        print *, " MenLoc = ", MenLoc, " is < 450 GB "
      
      if(opcB==1)then        !!! RHS
      
	    allocate (h(LDB,jnrhs),hteo(LDB,jnrhs),b(LDB,jnrhs),b0(LDB,jnrhs),D(m,jnrhs))
      ...
      elseif(opcB==2)then  !! direct gem sym matrix
         ...
         allocate (h(LDB,jnrhs),hteo(LDB,jnrhs),b(LDB,jnrhs),b0(LDB,jnrhs))
      ...
      elseif(opcB==3)then  !! b random
        ...
        !h=0.0d0 
            !print *, ""
            !print *, "+++++ Star   magmaf_OoC_DPoTRS nrhs=",jnrhs
            !T4=MPI_WTIME()
            T4 =omp_get_wtime()
        !!!call magmaf_dpotrs_cpu( 'l', n, jnrhs, hA, lda, hX, lda, info )
        !!!call magmaf_OoC_DPoTRS('L', N, jNRHS, A, LDA, B, LDA, INFO)  !! out of core
        call magmaf_OoC_DPoTRS('L', N, jNRHS, A, LDA, B, LDA, INFO,L,Nb,maxNRHS)  !! out of core
        
        nb_rhs(nkRHS)=nb
        L_rhs(nkRHS)=L
        max_rhs(nkRHS)=maxNRHS
        
        !print *, "L_rhs(nkRHS) = ",L_rhs(nkRHS)
        !print *, "max_rhs(nkRHS) = ", max_rhs(nkRHS)
        
        
            !T5=MPI_WTIME() 
            T5 =omp_get_wtime()
            !print *, "+++++ finish magmaf_OoC_DPoTRS"
            !print *, ""
            !print *, " time magmaPoTRS  ", T5-T4," s"
            tslv=T5-t4
            TIME_ITER_TRS_RHS(LNCH,nkRHS ) = tslv
...
!     Compute residual ||A * X  - B|| / ( ||X|| * ||A|| * eps * N )
        ...
        if(opcB==1 .or. opcB==2)deallocate(B0,B,Hteo,H)
   ...
   ENDDO !!    LUNCH
        ...
        !!!!   report
                  !!  fac 
        CALL CALC_AVERAGE_TIME( TIME_ITER_TRF, ITERATIONS, tfac )
	    ...
	    CALL CALC_AVERAGE_TIME( TIME_ITER_MMsyrk, ITERATIONS, tmmsyrk )
         ...
         !! facto
         Ofac=( 2*INT8(n)*INT8(n)*INT8(n) + 3*INT8(n)*INT8(n) + INT8(nc) )/6
         dOfac=dble(Ofac)
	...
	!! MM syrk
        ...
        !!      --------------------          RESULT              ------------------------------------
         ...
         do k=kmin,kmax,kstep   !!  jnrhs  !! loop 
            ...
            CALL CALC_AVERAGE_TIME( TIME_ITER_TRS_rhs(:,nkrhs), ITERATIONS, tslv )
            ...
            !write(NOUT,414) n,jNrhs,LDA,iterations,TESTS_PASSED,derror,RESID,SM_abs,tfac,tslv,tfac+tslv,&
            !      GflpSfac,GflpSslv,GflpST,GflpMMsyrk,GflpMM,dmen_trf+dMen_trf_work,dmen_trs+dMen_trs_work,MenLoc_rhs(nkRHS)
             write(NOUT,415) n,jNrhs,LDA,nb_rhs(nkRHS),max_rhs(nkRHS),L_rhs(nkRHS),iterations,TESTS_PASSED,derror,RESID,SM_abs,&
                    tfac,tslv,tfac+tslv,GflpSfac,GflpSslv,GflpST,GflpMMsyrk,GflpMM,&
                    dmen_trf+dMen_trf_work,dmen_trs+dMen_trs_work,MenLoc_rhs(nkRHS)
    ...
    !! Print header
        ...
        WRITE( NOUT, '(A)'    ) ''
        WRITE( NOUT, '(A)'    ) ''                  
            DEALLOCATE( TIME_ITER_TRF )
            DEALLOCATE( TIME_ITER_TRS )
            DEALLOCATE( TIME_ITER_TRS_rhs )
...
414   format((3I8,2I4,6ES11.3,8F9.2))  !! 
514   format((3(a8),2(a4),6(a11),8(a9)))  !! 

415   format((5I8,3I4,6ES11.3,5F9.2,2F10.2,1F9.2))  !! 
515   format((5(a8),3(a4),6(a11),5(a9),2(a10),1(a9)))  !!
